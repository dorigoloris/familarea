(function () {
  ...
  function openModal(options) {
    ...
    const fields = settings.fields || (settings.input ? [settings.input] : []);
    const isMultiField = Boolean(settings.fields);
    const inputs = [];
    activeModal = overlay;
    ...
    fields.forEach((fieldSettings, index) => {
      const field = createElement('div', 'confirm-modal-field'); const label = createElement('label', '', fieldSettings.label);
      const input = document.createElement('input'); input.id = `confirm-modal-input-${fieldSettings.name || index}`; input.type = fieldSettings.type || 'text'; input.value = fieldSettings.value || ''; input.placeholder = fieldSettings.placeholder || ''; input.autocomplete = fieldSettings.autocomplete || ''; input.required = Boolean(fieldSettings.required); label.htmlFor = input.id;
      inputs.push({ input, settings: fieldSettings, name: fieldSettings.name || String(index) }); field.append(label, input); dialog.appendChild(field);
    });
    ...
    return new Promise((resolve) => {
      ...
      function inputIsValid() {
        return inputs.every(({ input, settings: fieldSettings }) => (!input.required || input.value.trim() !== '') && (!fieldSettings.validate || fieldSettings.validate(input.value) !== false));
      }
      function updateSubmitState() { confirmButton.disabled = !inputIsValid(); }
      function submit() { if (inputs.some(({ input }) => !input.reportValidity()) || !inputIsValid()) return; confirmButton.disabled = cancelButton.disabled = closeButton.disabled = true; finish(isMultiField ? Object.fromEntries(inputs.map(({ input, name }) => [name, input.value])) : inputs[0]?.input.value ?? true); }
      function onKeyDown(event) {
        if (event.key === 'Escape') { event.preventDefault(); finish(null); return; }
        if (event.key === 'Enter' && inputs.some(({ input }) => document.activeElement === input)) { event.preventDefault(); submit(); return; }
        if (event.key !== 'Tab') return;
        const focusable = [closeButton, ...inputs.map(({ input }) => input), cancelButton, confirmButton]; const index = focusable.indexOf(document.activeElement);
      ...
      inputs.forEach(({ input }) => input.addEventListener('input', updateSubmitState)); updateSubmitState();
      overlay.addEventListener('click', (event) => { if (event.target === overlay) finish(null); }); document.addEventListener('keydown', onKeyDown);
      backgroundElements.forEach(({ element }) => { element.inert = true; element.setAttribute('aria-hidden', 'true'); }); document.body.classList.add('confirm-modal-open'); document.body.appendChild(overlay); (inputs[0]?.input || cancelButton).focus();
    });
  }
  ...
  function form(options) { return openModal({ ...options, fields: options.fields || [] }); }
  window.FamilAreaConfirm = { confirm, prompt, form };
}());
