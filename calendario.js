...
async function loadMonthSources() {
  ...
  updateVisibleCalendarItems(); renderMonth();
...
async function changeMonth(offset) {
  ...
  const results = await loadMonthSources();
  calendarMessage.textContent = updateMessage(results.occurrenceFailed, false, results.birthdayFailed);
}
async function loadCalendar() {
  ...
  renderUndatedActivities(); calendarContent.hidden = false; calendarMessage.textContent = updateMessage(activityFailed, eventFailed, monthResults.birthdayFailed);
}
previousMonthButton.addEventListener('click', () => { void changeMonth(-1); });
nextMonthButton.addEventListener('click', () => { void changeMonth(1); });
