...
begin
  ...
  if p_target_type='event' then return exists(select 1 from public.area_events e join public.area_memberships am on am.area_id=e.area_id and am.profile_id=v_profile_id and am.role in ('admin','member') where e.id=p_target_id and e.created_by_profile_id=v_profile_id); end if;
  if p_target_type='activity' then return exists(select 1 from public.area_activities a join public.area_memberships am on am.area_id=a.area_id and am.profile_id=v_profile_id and am.role in ('admin','member') where a.id=p_target_id and (a.created_by_profile_id=v_profile_id or(a.visibility='area' and am.role='admin'))); end if;
  return false;
end;
$$;
...
begin
  ...
  return exists(
    ...
    where x.id=p_attachment_id and (
      ...
      or (x.target_type='event' and exists(select 1 from public.area_memberships am left join public.event_participants ep on ep.event_id=e.id and ep.profile_id=v_profile_id where am.area_id=e.area_id and am.profile_id=v_profile_id and am.role in ('admin','member') and (e.visibility='area' or e.created_by_profile_id=v_profile_id or(e.visibility='creator_participants' and ep.profile_id is not null))))
      or (x.target_type='activity' and exists(select 1 from public.area_memberships am where am.area_id=a.area_id and am.profile_id=v_profile_id and am.role in ('admin','member') and (a.visibility='area' or a.created_by_profile_id=v_profile_id or(a.visibility='creator_assignees' and exists(select 1 from public.activity_assignees aa where aa.activity_id=a.id and aa.profile_id=v_profile_id)))))
    )
  );
end;
$$;
...
returns boolean language sql security definer set search_path=public,pg_temp as $$
  select public.can_manage_attachment_target(x.target_type,coalesce(x.deadline_id,x.event_id,x.activity_id)) from public.attachments x where x.id=p_attachment_id;
$$;
...
begin
  ...
  return exists(select 1 from public.attachments x where x.storage_path=p_storage_path and public.can_view_attachment(x.id));
end;
$$;
...
begin
  if p_storage_path is null or btrim(p_storage_path)='' then return false; end if;
  return exists(select 1 from public.attachments x where x.storage_path=p_storage_path and public.can_manage_attachment(x.id));
end;
$$;

create function public.create_my_attachment(p_target_type text,p_target_id uuid,p_original_filename text,p_mime_type text,p_byte_size bigint)
returns table(id uuid,storage_path text) language plpgsql security definer set search_path=public,pg_temp as $$
declare v_profile_id uuid; v_id uuid:=gen_random_uuid(); v_extension text; v_filename text:=btrim(coalesce(p_original_filename,''));
begin
  if auth.uid() is null then raise exception 'Utente non autenticato'; end if;
  select p.id into v_profile_id from public.profiles p where p.user_id=auth.uid(); if v_profile_id is null then raise exception 'Profilo non trovato per l''utente corrente'; end if;
  if p_target_type not in ('deadline','event','activity') or p_target_id is null then raise exception 'Destinazione allegato non valida'; end if;
  ...
  v_extension:=case p_mime_type when 'application/pdf' then 'pdf' when 'image/jpeg' then 'jpg' when 'image/png' then 'png' when 'image/webp' then 'webp' end;
  id:=v_id; storage_path:='attachments/'||v_id::text||'/file.'||v_extension;
  insert into public.attachments(id,target_type,deadline_id,event_id,activity_id,storage_path,original_filename,mime_type,byte_size,uploaded_by_profile_id)
  values(v_id,p_target_type,case when p_target_type='deadline' then p_target_id end,case when p_target_type='event' then p_target_id end,case when p_target_type='activity' then p_target_id end,storage_path,v_filename,p_mime_type,p_byte_size,v_profile_id);
  return next;
end;
$$;

create function public.get_my_attachments(p_target_type text,p_target_id uuid)
returns table(id uuid,original_filename text,mime_type text,byte_size bigint,created_at timestamptz,storage_path text)
language plpgsql security definer set search_path=public,pg_temp as $$
begin
  if p_target_type not in ('deadline','event','activity') or p_target_id is null then raise exception 'Destinazione allegato non valida'; end if;
  return query select x.id,x.original_filename,x.mime_type,x.byte_size,x.created_at,x.storage_path from public.attachments x where x.target_type=p_target_type and coalesce(x.deadline_id,x.event_id,x.activity_id)=p_target_id and public.can_view_attachment(x.id) order by x.created_at,x.id;
end;
$$;
...
language plpgsql security definer set search_path=public,pg_temp as $$
begin
  if not public.can_view_attachment(p_attachment_id) then raise exception 'Allegato non trovato o non accessibile'; end if;
  return query select x.storage_path,x.original_filename,x.mime_type from public.attachments x where x.id=p_attachment_id;
end;
$$;
...
returns void language plpgsql security definer set search_path=public,pg_temp as $$
begin
  if not public.can_manage_attachment(p_attachment_id) then raise exception 'Allegato non trovato o non gestibile'; end if;
  delete from public.attachments where id=p_attachment_id;
end;
$$;

insert into storage.buckets(id,name,public,file_size_limit,allowed_mime_types)
values('familarea-attachments','familarea-attachments',false,2097152,array['application/pdf','image/jpeg','image/png','image/webp']::text[])
...
create policy familarea_attachments_select_visible on storage.objects for select to authenticated using (bucket_id='familarea-attachments' and public.can_view_attachment_storage_path(name));
create policy familarea_attachments_insert_authorized on storage.objects for insert to authenticated with check (bucket_id='familarea-attachments' and public.can_manage_attachment_storage_path(name));
create policy familarea_attachments_delete_authorized on storage.objects for delete to authenticated using (bucket_id='familarea-attachments' and public.can_manage_attachment_storage_path(name));
